...
module.exports.run = async (bot, message, args) => {
  ...
  if(bUser === message.author) return message.channel.send("Are you retarded? Why do you want to ban yourself?")
   ...
   if(!bReason) return message.channel.send("Please provide a reason!")
    if(!message.member.hasPermission("BAN_MEMBERS")) return message.channel.send("<:tickNo:495100045863944192> **| You don't have `BAN_MEMBERS` permissons.**");
    ...
    if(!incidentchannel) return message.channel.send("<:tickNo:495100045863944192> **| Can't find `mod-log` channel.**");
    ...
    .addField("Moderator", message.author.tag)
    .addField("Reason", `${bReason ? bReason : "None."}`)
  message.channel.send("<:tickYes:495100878051737610> **| That member has been banned.**")
  bUser.ban(bReason)
